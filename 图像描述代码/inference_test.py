    ...
    for imgs, cptns_all, lens in pb:
        ...
        while current_preds.size(1) <= (
            ...
            if torch.any(seqs_end):
                ...
                attn_joint = attns[-1].mean(dim=1)         # [k, 265, 265]
                # 假设第 265 个维度中前 256 是图像 patch，后面的是 token
                attn_to_img = attn_joint[:, :, :256]       # [k, 265, 256]
                # 从中取出每个 token 的注意力图（最后一个 token） → 第 264 行（0-based）
                attn_img_last_token = attn_to_img[:, -1]   # [k, 256]
                attns = attn_img_last_token.view(k, h, w)  # [k, h, w]
        ...
        pb.set_description(
            f'BLEU1: Cur={selected_data["bleu1"][-1]:.4f}, '
            f'Mean={mean(selected_data["bleu1"]):.4f}±{pstdev(selected_data["bleu1"]):.2f} | '
            f'BLEU4: Cur={selected_data["bleu4"][-1]:.4f}, '
            f'Mean={mean(selected_data["bleu4"]):.4f}±{pstdev(selected_data["bleu4"]):.2f} | '
            f'METEOR: Cur={selected_data["meteor"][-1]:.4f}, '
            f'Mean={mean(selected_data["meteor"]):.4f}±{pstdev(selected_data["meteor"]):.2f}'
        )

    pb.close()
    print("\nFinal Metrics:")
    print(f'BLEU-1: Mean={mean(selected_data["bleu1"]):.4f} ± {pstdev(selected_data["bleu1"]):.4f}')
    print(f'BLEU-4: Mean={mean(selected_data["bleu4"]):.4f} ± {pstdev(selected_data["bleu4"]):.4f}')
    print(f'METEOR: Mean={mean(selected_data["meteor"]):.4f} ± {pstdev(selected_data["meteor"]):.4f}')
