...
class DecoderLayer(nn.Module):
    ...
    def forward(self, tgt: Tensor, memory: Tensor, tgt_mask: Tensor, tgt_pad_mask: Tensor) -> Tuple[Tensor, Tensor]:
        ...
        tgt = self.norm1(tgt)
        attn_output, _ = self.self_attn(tgt, tgt, tgt, attn_mask=tgt_mask, key_padding_mask=tgt_pad_mask)
        tgt = tgt + self.dropout(attn_output)
        

        # Cross Attention
        tgt = self.norm2(tgt)
        attn_output, attn_weights = self.cross_attn(tgt, memory, memory)
        tgt = tgt + self.dropout(attn_output)
...
class Decoder(nn.Module):
    ...
    def __init__(self, vocab_size, d_model, num_layers, max_len, dropout, pad_id, num_heads=12):
        ...
        self.layers = nn.ModuleList([
            DecoderLayer(embed_dim=d_model, num_heads=num_heads, ff_dim=3072, dropout=dropout)
